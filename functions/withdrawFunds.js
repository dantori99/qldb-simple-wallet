...
const { withdrawBalance } = require('./helper/utils');
...
const handler = async (event) => {
  ...
  const { amountToWithdraw } = JSON.parse(event.body);

  try {
    // updateFunc
    const response = await withdrawBalance(guid, amountToWithdraw, logger);
    // getTheData
